...
func (s *Service) reconstructRow(topic string, groupID []byte, rowIndex uint64, phase int) {
	...
	start := time.Now()
	// What the row held before recovery is the advertisement gate's input: a cell that arrived
	// row-wise had a source in the subnet, so its column has a relay and needs nothing from us; a
	// cell recovery had to supply is the observable trace of a missing relay (design.md section 6
	// item 16). Snapshot it before RecoverRow fills everything in.
	...
	// Then advertise it to the columns we do *not* custody and had to recover. Serving the row
	// helps the row subnet and our own columns; this is the step EIP-8371 expects to turn one
	// node's reconstruction into relief for column subnets whose own path failed.
	s.crossForwardRecoveredRow(topic, row, heldBefore)
}
