...
func TestSegmentsForEnvelopeRoundTrip(t *testing.T) {
	...
	auth := &ethpb.PayloadSegmentAuth{
		...
		HashId:      uint32(segments.HashSHA256),
		Slot:        slot,
	}

	t.Run("publisher builds segments a receiver authenticates", func(t *testing.T) {
		segs, err := SegmentsForEnvelope(signed, auth)
		require.NoError(t, err)
		...
		var out []byte
		for _, s := range segs {
			m, h, err := segments.UnmarshalSegmentMessage(s.Segment)
			require.NoError(t, err)
			got, err := r.Add(h, m)
			require.NoError(t, err)
			if got != nil {
	...
	t.Run("a different segmentation still produces a well-formed group", func(t *testing.T) {
		other := &ethpb.PayloadSegmentAuth{
			SegmentSize: descriptor.SegmentSize / 2,
			HashId:      auth.HashId,
			Slot:        auth.Slot,
		}
	...
	t.Run("unknown hash id rejected", func(t *testing.T) {
		bad := &ethpb.PayloadSegmentAuth{SegmentSize: auth.SegmentSize, HashId: 200, Slot: auth.Slot}
		_, err := SegmentsForEnvelope(signed, bad)
	...
	t.Run("invalid segment size rejected", func(t *testing.T) {
		for _, size := range []uint32{0, segments.MaxSegmentSize + 1} {
			bad := &ethpb.PayloadSegmentAuth{SegmentSize: size, HashId: auth.HashId, Slot: auth.Slot}
			_, err := SegmentsForEnvelope(signed, bad)
			require.ErrorIs(t, err, segments.ErrSegmentSize)
		}
	})

	t.Run("nil inputs rejected", func(t *testing.T) {
		_, err := SegmentsForEnvelope(nil, auth)
		require.ErrorIs(t, err, ErrEnvelopeMalformed)
		_, err = SegmentsForEnvelope(signed, nil)
		require.ErrorIs(t, err, ErrEnvelopeMalformed)
	})
