...
// RowRequestDeferral is D17 clause 1: how long a new row's default request policy stays silent,
// yielding right-of-way to the column path, which EIP-8371 line 171 makes authoritative and line
// 56 already permits deferring behind ("a node MAY choose to delay the request of cells from
// rows"). R18 measured what zero costs at the realistic mixed-custody shape: ~+0.5 s of
// column-completion p50 in every custody class, on the wall clock, 0 of 15 paired cells
// favouring the row axis. Only requests wait; availability is announced on time, and explicit
// request bitmaps (the pull arm, recovered rows) bypass the hold.
//
// One second is R19's choice (2026-08-31, three paired seeds, each effect ~10x the wall noise
// band): 0 ms costs ~+510 ms of column p50 at the mixed shape, 500 ms still ~+324, 1000 ms +18
// (noise), 2000 ms buys nothing more. The knee sits at about the column path's p50 -- storm-era R7
// needed its p95, but the announce repairs since removed most of the contention. The right
// production value is a property of the network's column path, not of the protocol, so this stays
// a var; node-side it should become a slot-fraction config. A refinement deliberately not built
// yet: a node with reconstruction duty on the row might skip the hold (D17 clause 4), measured
// before adopted.
var RowRequestDeferral = time.Second
...
func (p *PartialColumnBroadcaster) applyRowRequestPolicy(row *blocks.PartialDataRow) {
	if RowRequestDeferral > 0 {
		row.DeferRequestsUntil(p.now().Add(RowRequestDeferral))
	}
	if p.rowCallbacks == nil {
		return
	}
