...
// CrossForwardEagerPush selects the wire behaviour of CrossForwardRow. False (the default, per
// design.md section 6 item 16): the built columns are *registered* -- state stored, requests
// served -- and their announcement rides heartbeat gossip; nothing is sent at forward time. True
// is the original eager push, kept as R17's comparison arm: publish immediately, paying a
// first-contact metadata burst across every joined topic (R16 measured it at ~26k RPCs per slot
// at 128 nodes, delivering nothing the column path did not).
var CrossForwardEagerPush = false
...
func (p *PartialColumnBroadcaster) crossForwardRowOnLoop(req crossForwardRow) error {
	...
	*req.pushed = len(topics)
	...
	return p.publishColumns(func(yield func(string, blocks.PartialDataColumn) bool) {
		for i, topic := range topics {
			if !yield(topic, pushColumns[i]) {
				return
			}
		}
	}, !CrossForwardEagerPush)
}
