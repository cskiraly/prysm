...
func (p *PartialColumnBroadcaster) armRowClaimWake(topic string, groupID []byte, row *blocks.PartialDataRow) {
	...
	// A pending request deferral (D17 clause 1) is a publish deadline of the same kind: when it
	// expires the requests appear, and something must run a publish to put them on the wire.
	if deferred := row.RequestsDeferredUntil(); !deferred.IsZero() && p.now().Before(deferred) && !row.IsComplete() {
		if !ok || deferred.Before(deadline) {
			deadline, ok = deferred, true
		}
	}
	if !ok {
		return
