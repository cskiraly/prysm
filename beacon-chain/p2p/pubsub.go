...
func (s *Service) JoinTopic(topic string, opts ...pubsub.TopicOpt) (*pubsub.Topic, error) {
	...
	if _, ok := s.joinedTopics[topic]; !ok {
		...
		// RowDAS rows have no full-message form at all, so requesting partial messages is not
		// an optimisation here -- without it the topic carries nothing.
		if strings.Contains(topic, GossipDataRowMessage) && s.partialColumnBroadcaster != nil {
			opts = append(opts, pubsub.RequestPartialMessages())
			log.WithField("topic", topic).Debug("Joining data row topic with partial messages")
		}
		// Variant B rides the envelope topic rather than a topic of its own. Requesting
		// partial messages here is what makes the representation a per-link choice: peers
		// that also request them exchange segments and are skipped by the full-message path,
		// and peers that do not still receive the whole envelope on this same topic.
		// Matched with both separators, not as a substring: "execution_payload" is a prefix
		// of both "execution_payload_bid" and "execution_payload_segment", so a Contains on
		// the bare name would silently request partial messages on three topics.
		if strings.Contains(topic, "/"+GossipExecutionPayloadEnvelopeMessage+"/") && s.segmentBroadcaster != nil {
			opts = append(opts, pubsub.RequestPartialMessages())
			s.segmentBroadcaster.ServeTopic(topic)
			log.WithField("topic", topic).Debug("Joining execution payload envelope topic with partial messages")
		}
...
func (s *Service) pubsubOptions() []pubsub.Option {
	...
	if s.segmentBroadcaster != nil {
		psOpts = s.segmentBroadcaster.AppendPubSubOpts(psOpts)
	}
...
// GossipSubParams exposes Prysm's gossipsub parameters so a test harness can run the same
// overlay and gossip configuration the production node does, rather than library defaults.
func GossipSubParams() pubsub.GossipSubParams {
	return pubsubGossipParam()
}
