...
//
// defaultInSubnetDegree is the in-subnet neighbour target when an experiment does not choose one:
// 6, matching Prysm's --minimum-peers-per-subnet default, which sync actively dials toward -- the
// plan's fidelity item (plan-repair.md section 7). The old value of 4 was random-overlap
// arithmetic, and R18's heterogeneous shape found its edge: one column subscriber can sit at mesh
// size 0 for minutes when its few in-subnet neighbours cluster on saturated hubs.
// ROWDAS_IN_SUBNET_DEGREE overrides. Changing this changes every graph: a measurement epoch.
func defaultInSubnetDegree() int { return envInt("ROWDAS_IN_SUBNET_DEGREE", 6) }
...
func newNetwork(t *testing.T, o networkOpts) (*rowNetwork, func()) {
	...
	if o.rowMembers == nil {
		...
		if o.subnetAware && members < n {
			...
			if inSubnet == 0 {
				inSubnet = defaultInSubnetDegree()
			}
