...
func runR2Arm(t *testing.T, arm r2Arm, seed uint64) r2Result {
	...
	expected := r2Nodes() * r2ColumnsPerNode()
	...
	if arm.rows && arm.push {
		...
		for i, node := range nw.nodes {
			...
			_, err = node.broadcaster.CrossForwardRow(context.Background(), nw.topic, row, nonCustodiedOf(node.columns))
			require.NoError(t, err)
		}
	}
