...
// exactly as Version splits it) plus parity segments, any K of the total recovering the
...
// coded descriptor authenticates K and N with the same fields, and the same commitment, as a
// plain one.
//
// The code is the textbook systematic Vandermonde construction over GF(2^8) with table
// arithmetic, kept dependency-free. It is the slowest correct implementation: a SIMD library
// such as klauspost/reedsolomon runs the same arithmetic one to two orders of magnitude
// faster, and is the production choice once the shape of the code is settled.
