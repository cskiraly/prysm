...
type pendingNews struct {
	...
	availPending bool
	// availSince is when the pending growth was first noted, the trailing-edge arm's window
	// anchor (blocks.AvailabilityLeadingEdge = false, an R3(b) sweep configuration). The leading
	// edge ignores it.
	availSince time.Time
...
// noteAvailPending records that availability grew for p since it was last told. It returns the two
// window anchors: when the peer was last told (the leading edge's; zero means never told, which the
// caller treats as quiet) and when this growth was first noted (the trailing edge's, set to now on
// the first note and kept until the news is delivered).
func (c *requestClaims) noteAvailPending(p peer.ID, now time.Time) (lastTold, availSince time.Time) {
	c.mu.Lock()
	defer c.mu.Unlock()

	n := c.pending[p]
	n.availPending = true
	if n.availSince.IsZero() {
		n.availSince = now
	}
	c.pending[p] = n

	return n.lastTold, n.availSince
}
...
func (c *requestClaims) earliestDeadline() (time.Time, bool) {
	...
	cancelDelay := cancellationMaxDelay()
	for p, n := range c.pending {
		if !n.cancelSince.IsZero() {
			consider(n.cancelSince.Add(cancelDelay))
		}
		if n.availPending {
			// The same anchor and per-peer window the due check in partialForPeer uses, or the
			// wake-up and the decision disagree about the same peer.
			anchor := n.lastTold
			if !AvailabilityLeadingEdge {
				anchor = n.availSince
			}
			consider(anchor.Add(availabilityDelayFor(p)))
		}
	}
