...
//
// A var so R3 can sweep it; 100 ms is the throttle's old window, kept so the two can be compared.
var RequestAvailabilityMaxDelay = 100 * time.Millisecond
...
func partialForPeer(
	...
	if sentMeta != nil {
		...
		if availGrew && holds && !availDue {
			...
			// one, which the trailing-edge version of this got wrong.
			last := parts.claims().noteAvailPending(peerID)
			availDue = last.IsZero() || !now.Before(last.Add(availabilityMaxDelay()))
		}
